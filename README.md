...
- **Scatter / regression panel** — compares the crude rate for the selected injury type and year against an environmental variable (Annual Mean Temperature °F or Annual Total Precipitation inches), sourced from NOAA nClimDiv. Works at both state and county level. Each point is one state or county; the panel shows OLS slope, R², Pearson r, and p-value with plain-language interpretations specific to the selected injury type and level. A three-tier data-sufficiency system gates the statistics:
  - **n < 10**: regression blocked — "Not enough data for meaningful statistical analysis."
  - **10 ≤ n < 30**: statistics shown with a yellow "Limited data" caution banner.
  - **n ≥ 30**: full analysis. Thresholds follow Green (1991, *Multivariate Behavioral Research* 26(3), 499–510), who recommends N ≥ 50 + 8m (m = predictors; here m = 1, so N ≥ 58 is ideal). Only counties with a valid, non-suppressed crude rate **and** a known population are counted toward n.

**Map data categories (Standard view):** The choropleth now distinguishes four county/state states:

| Legend label | Meaning | Color |
|---|---|---|
| Rate bins (e.g. "2.3–5.7") | Valid crude rate, binned by quintile | Light → dark blue |
| Zero Deaths | 0 reported deaths with known population | White |
| Unreliable (1–9 deaths) | CDC-suppressed count; rate stored as −1.0 sentinel | Amber |
| Not Available | Missing rate or unknown population | Gray |
